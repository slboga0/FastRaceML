...
from modeling.model_training import binary_regression_classifier, lgb_classifier, random_forest_classifier, remap_labels
...
def preprocess_data(X, strategy='mean'):
...
def main():
    ...
    merged_df = merged_df.dropna(subset=['res_finish_position'])

    # Convert the column to numeric; non-convertible values become NaN
    merged_df['res_finish_position'] = pd.to_numeric(merged_df['res_finish_position'], errors='coerce')

    # Optionally, check how many values became NaN
    print(merged_df['res_finish_position'].isna().sum())
    ...
    y_train_mapped, mapping = remap_labels(y_train)
    y_val_mapped, _ = remap_labels(y_val)
    y_test_mapped, _ = remap_labels(y_test)

    model, feature_importance = lgb_classifier(X_train, y_train_mapped, X_val, y_val_mapped, X_test, y_test_mapped)
